...
let analyser_param info =
  ...
  let analyser (Ast.AstPlacement.Programme (vars, fonctions, (prog, _))) =
    ...
    let global_vars = List.map (fun (Ast.AstPlacement.Globale (info, _, _)) ->
      match Tds.info_ast_to_info info with
      | InfoGlobal (n, _, d) -> (n, (d, "SB"))
      | _ -> failwith "Erreur : une variable globale doit être de type InfoGlobal"
    ) vars in
  
    (* Analyse des instructions principales *)
    let main_vars = List.flatten (List.map analyser_instruction prog) in
  
    (* Analyse des fonctions *)
    let functions_vars = List.flatten (List.map analyser_fonction fonctions) in
  
    (* Résultat final combiné *)
    ("main", global_vars @ main_vars) :: functions_vars
