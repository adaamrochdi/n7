...
val string_of_varglobale : A.varglobale -> string
...
let string_of_varglobale v= 
  begin 
    match v with 
    |Globale(t,n,e)-> "static"^(string_of_type t)^n^"="^(string_of_expression e)
  end 
  ...
  let string_of_programme (Programme (v, fonctions, instruction)) =
    (List.fold_right (fun vg acc -> (string_of_varglobale vg) ^ "\n" ^ acc) v "")^
