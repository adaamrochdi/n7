...
struct
  ...
  let encoder_lettre t =
    if t < 'a' || t > 'z' then 0
    else
      let rec trouver_touche e = 
        match e with
        | [] -> 0
        | (touche, lettres) :: q ->
            if List.mem t lettres then touche
            else trouver_touche q
        in trouver_touche E.map
  ...
  let encoder_mot mot =
    let rec aux acc i =
      if i < 0 then acc
        ...
        let touche = encoder_lettre (String.get mot i) in
        aux (touche :: acc) (i - 1)
    in
  ...
  let ajouter d mot = S.ajouter (encoder_mot mot) mot d
  ...
  let decoder_mot  l d = S.chercher l d 
  ...
  (* Calcule le nombre maximum de mots associés à un même code dans un
     dictionnaire *)
  let max_mots_code_identique  d = S.max_mots_code_identique d
  ...
  let prefixe  pref d = S.prefixe pref d 
