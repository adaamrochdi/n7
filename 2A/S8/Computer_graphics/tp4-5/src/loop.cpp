                     ...
                     std::vector<vec3d>& destNorm)         //!< the new normals
{
...

    {
        ...
        //*********************************************************************
        destNorm[f.v1] += angleAtVertex(destVert[f.v1], destVert[f.v2], destVert[f.v3]) * n;
        destNorm[f.v2] += angleAtVertex(destVert[f.v2], destVert[f.v3], destVert[f.v1]) * n;
        destNorm[f.v3] += angleAtVertex(destVert[f.v3], destVert[f.v1], destVert[f.v2]) * n;
                     ...
                     EdgeList& newVertList)
{
    ...
    if (!newVertList.contains(e))
    {
        ...
        if (!isBoundaryEdge(e, mesh, oppV1, oppV2))
        {
            ...
            //*********************************************************************
            nvert = (vertList[e.first] * 3.0f/8.0f + 
                vertList[e.second] * 3.0f/8.0f +
                vertList[oppV1] * 1.0f/8.0f + 
                vertList[oppV2] * 1.0f/8.0f);
