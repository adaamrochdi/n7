...
def bezier_bernstein(x_ctrl_pts: list[float], y_ctrl_pts: list[float], list_t: list[float]) -> np.ndarray:
    ...
    for i in range(deg_max + 1):
        result[:, 0] += x_ctrl_pts[i] * base[i][:, 1]
        result[:, 1] += y_ctrl_pts[i] * base[i][:, 1]
...
def subdivision(x: list, y: list) -> tuple:
    ...
    for k in range(n-1):
        ...
        for i in range (len(points_x)-1):
            mid_x = (points_x[i] + points_x[i+1]) / 2
            mid_y = (points_y[i] + points_y[i+1]) / 2
            nx.append(mid_x)
            ny.append(mid_y)
...
def approximation_surface(xx, yy, zz, list_tt, nb_grid_pts):
    ...
    for i in range (1,n):
        approx_1=np.zeros((n,3))
        approx_2=np.zeros((n,3))
        
        for j in range (1,n):
            approx[:,1] =de_casteljau(
