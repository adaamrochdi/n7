...
% Parametres :
N = 200;					% Nombre de disques d'une configuration
...
for i = 1:N
    bool = false;
    while ~bool
        c_i = [nb_colonnes * rand, nb_lignes * rand]; % Tirage aléatoire
        bool = true;
        for j = 1:i-1
            if norm(c(j,:) - c_i) < sqrt(2) * R
                bool = false;
            end
        end
    end
...
for q = 1:q_max
	...
	if I_moyen_nouv>I_moyen_cour 
        bool =true;
        for j = 1:N
            if j ~= i && norm(c(j,:) - c_alea) < sqrt(2) * R
            valide = false;
            end
        end
        if bool
