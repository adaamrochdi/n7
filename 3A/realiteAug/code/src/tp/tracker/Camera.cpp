...
void Camera::getOGLProjectionMatrix(float* proj, float znear, float zfar) const
{
    ...
    proj[0] = static_cast<float>(2.f * matK.at<double>(0, 0)) / static_cast<float>(imageSize.width);
    proj[1 * 4 + 0] = static_cast<float>(2 * matK.at<double>(0, 1)) / static_cast<float>(imageSize.width);
    proj[2 * 4 + 0] = -static_cast<float>(imageSize.width - 2 * matK.at<double>(0, 2)) / static_cast<float>(imageSize.width);
    proj[3 * 4 + 0] = 0.f;

    proj[0 * 4 + 1] = 0.f;
    // minus -(float)(2*matK.at<double>(1,1))/imageSize.height;
    proj[1 * 4 + 1] = -static_cast<float>(2.f * matK.at<double>(1, 1)) / static_cast<float>(imageSize.height);
    proj[2 * 4 + 1] = -static_cast<float>(-imageSize.height + 2.f * matK.at<double>(1, 2)) / static_cast<float>(imageSize.height);
    proj[3 * 4 + 1] = 0.f;
