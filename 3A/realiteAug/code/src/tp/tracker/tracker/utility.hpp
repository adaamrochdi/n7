...
void calcChessboardCorners(const cv::Size& boardSize,
                           ...
                           Pattern patternType = Pattern::CHESSBOARD);
...
void filterVector(std::vector<T>& inout, const std::vector<int>& idx)
{
    ...
    {
        assert(static_cast<std::size_t>(i) < inout.size() && i >= 0 && "Index out of range in filterVector");
        temp.push_back(static_cast<T>(inout[i]));
    }
 ...
 * @param[in,out] capture The video stream, it must be already open
