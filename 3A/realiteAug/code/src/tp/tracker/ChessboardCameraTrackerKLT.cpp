  ...
  cv::Mat& view, cv::Mat& pose, const Camera& cam, const cv::Size& boardSize, const Pattern& pattern)
{
    ...
    const Mat temp = view.clone(); // used for correcting the optical distortion
    ...
    else
    {
        ...
        const TermCriteria termcrit(TermCriteria::MAX_ITER | TermCriteria::EPS, 20, 0.03);
        ...
        // status will have the same length as currPts and is 0 if the
        ...
        // resize the two vectors to the size k, the number of "well" tracked features
        _corners.resize(k);
