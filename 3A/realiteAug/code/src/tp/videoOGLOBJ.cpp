...
void DrawPoints(float scale)
{
    ...
    for(int i = 0; i < 6; i++)
    {
        ...
        for(int j = 0; j < 9; j++)
            glVertex3f(static_cast<float>(j), static_cast<float>(i), 0.f);
    }
...
void glInit()
{
    ...
    //******************************************************************
    const GLfloat gLightPos[] = {100.f, 200.f, -200.f, .0f};
    ...
    //******************************************************************
    const GLfloat mat_ambient[] = {.7f, .7f, .7f, 1.f};
    const GLfloat mat_diffuse[] = {.8f, .8f, .8f, 1.f};
    const GLfloat mat_specular[] = {1.f, 1.f, 1.f, 1.f};
...
void displayFunc()
{
    ...
    if(drawTeapot)
        ...
        glmDraw(model, GLM_SMOOTH | GLM_TEXTURE | GLM_MATERIAL);//  | GLM_MATERIAL);
...
int main(int argc, char** argv)
{
    ...
    [[maybe_unused]] auto pattern = Pattern::CHESSBOARD;
    ...
    Mat dummyMatrix = (Mat_<float>(4, 4) << 0.4830f,
                       -0.8756f,
                       0.0077f,
                       125.93854f,
                       0.8365f,
    ...
    while(!gFinished)
    {
        if(!stop)
        {
            ...
            if(tracker.process(view0, cameraPose, cam, boardSize, pattern))
            {
                ...
                gModelViewMatrix = reinterpret_cast<float*>(Mat(dummyMatrix.t()).data);
            }
